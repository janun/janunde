...
class SeminarForm(BetterForm):
    def __init__(self, *args, **kwargs):
        ...
        if richtlinie:
            self.fields["read_richtlinie"].label = self.fields[
                "read_richtlinie"
            ].label.replace(
        ...
        if datenschutz:
            self.fields["read_datenschutz"].label = self.fields[
                "read_datenschutz"
            ].label.replace(
    ...
    location = forms.CharField(
        label="Ort",
        help_text="Wo findet das Seminar statt? Bsp.: <em>Jugendumweltbüro, Goebenstr. 3a, 30161 Hannover</em>",
        widget=forms.TextInput(attrs={"class": "form__input--full"}),
    )
    ...
    contact_mail = forms.EmailField(
        label="E-Mail-Adresse", widget=forms.EmailInput(attrs={"size": 25}),
    )
    ...
    publish = forms.BooleanField(label="veröffentlichen?", initial=True, required=False)
